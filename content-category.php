...
<?php
$milkit_post_module_cols = ( get_option( 'milkit_opt_category_post_columns' ) ) ? get_option( 'milkit_opt_category_post_columns' ) : 'post-module-1cols';
...
<article <?php post_class( $milkit_post_module_classes ); ?> itemscope="itemscope" itemtype="http://schema.org/Article">
	...
	<?php if ( ! ( get_option( 'milkit_opt_category_post_thumbnail' ) == 'false' ) ) {
	...
	<div class="post-module-content">
		...
		<header class="entry-header">
			...
			<?php if ( ! ( get_option( 'milkit_opt_category_post_cat_icon' ) == 'false' ) ) {
				milkit_entry_cats();
			} ?>
			<?php the_title( sprintf( '<h2 itemprop="name" class="entry-title"><a itemprop="url" href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h2>' ); ?>
		</header><!-- .entry-header -->

		<?php if ( get_option( 'milkit_opt_category_post_excerpt' ) != 'nothing' ) : ?>

		<?php /* Display excerpt only if enabled */ ?>
		<?php if ( get_option( 'milkit_opt_category_post_excerpt' ) == 'excerpt' ) : ?>
		...
		<footer class="entry-footer">
			...
			<div class="author-date">
				<?php
				$milkit_date_vis_var = ( get_option( 'milkit_opt_category_post_date' ) == 'false' ) ? true : false;
				$milkit_author_vis_var = ( get_option( 'milkit_opt_category_post_author' ) == 'false' ) ? true : false;
				milkit_post_date( $milkit_date_vis_var );
				milkit_post_author( $milkit_author_vis_var );
				?>
			</div><!-- .author-date -->
			<div class="comments-views">
				<?php if ( ! ( get_option( 'milkit_opt_category_post_comments_icon' ) == 'false' ) ) {
					milkit_post_comments_count();
				}
				if ( ( get_option( 'milkit_opt_check_post_counter' ) == 'true' ) && ( get_option( 'milkit_opt_category_post_views_icon' ) == 'true' ) ) {
					do_action( 'milkit_display_post_views', get_the_ID() );
				} ?>
			</div><!-- .comments-views -->
			<?php if ( ! ( get_option( 'milkit_opt_category_post_review_icon' ) == 'false' ) ) {
