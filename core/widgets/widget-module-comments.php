...
class Milkit_Widget_Module_Comments extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'milkit_widget_module_comments',
			'description' => __( 'Your site\'s Posts ordered by comment count.', 'milkit' )
		);
		parent::__construct( 'milkit_widget_module_comments', __( 'Milkit Post Modules - By Comments', 'milkit' ), $widget_ops );
		$this->alt_option_name = 'milkit_widget_module_comments';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'milkit_widget_module_comments', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'milkit_widget_module_comments', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'milkit_widget_module_comments' ] ) ) {
			delete_option( 'milkit_widget_module_comments' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'milkit_widget_module_comments', 'widget' );
	}
...
add_action( 'widgets_init', 'milkit_widget_module_comments' );

function milkit_widget_module_comments() {
	register_widget( 'Milkit_Widget_Module_Comments' );
}
