...
class Milkit_Widget_Module_Tag extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'milkit_widget_module_tag',
			'description' => __( 'Your site\'s Posts filtered by tag.', 'milkit' )
		);
		parent::__construct( 'milkit_widget_module_tag', __( 'Milkit Post Modules - By Tag', 'milkit' ), $widget_ops );
		$this->alt_option_name = 'milkit_widget_module_tag';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'milkit_widget_module_tag', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'milkit_widget_module_tag', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'milkit_widget_module_tag' ] ) ) {
			delete_option( 'milkit_widget_module_tag' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'milkit_widget_module_tag', 'widget' );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'tag' ) ); ?>"><?php _e( 'Tag ID(s):', 'milkit' ); ?></label>
			...
			<br>
			<small><?php _e( 'Type a list of tag IDs separated by commas (eg. 1,5,8).', 'milkit' ); ?></small>
		</p>
...
add_action( 'widgets_init', 'milkit_widget_module_tag' );

function milkit_widget_module_tag() {
	register_widget( 'Milkit_Widget_Module_Tag' );
}
