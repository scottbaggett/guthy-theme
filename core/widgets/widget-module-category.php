...
class Milkit_Widget_Module_Category extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'milkit_widget_module_category',
			'description' => __( 'Your site\'s Posts filtered by category.', 'milkit' )
		);
		parent::__construct( 'milkit_widget_module_category', __( 'Milkit Post Modules - By Category', 'milkit' ), $widget_ops );
		$this->alt_option_name = 'milkit_widget_module_category';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'milkit_widget_module_category', 'widget' );
		}
		...
		if ( $order == 'comments' ) {
			...
			$r_args[ 'meta_key' ] = '_milkit_framework_post_views_count';
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'milkit_widget_module_category', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'milkit_widget_module_category' ] ) ) {
			delete_option( 'milkit_widget_module_category' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'milkit_widget_module_category', 'widget' );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php _e( 'Category ID(s):', 'milkit' ); ?></label>
			...
			<br>
			<small><?php _e( 'Type a list of category IDs separated by commas (eg. 1,5,8).', 'milkit' ); ?></small>
		</p>

		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('order') ); ?>"><?php _e( 'Order:', 'milkit' ); ?></label>
...
add_action( 'widgets_init', 'milkit_widget_module_category' );

function milkit_widget_module_category() {
	register_widget( 'Milkit_Widget_Module_Category' );
}
