...
class Milkit_Widget_Module_IDs extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'milkit_widget_module_ids',
			'description' => __( 'Your site\'s Posts filtered by IDs.', 'milkit' )
		);
		parent::__construct( 'milkit_widget_module_ids', __( 'Milkit Post Modules - By IDs', 'milkit' ), $widget_ops );
		$this->alt_option_name = 'milkit_widget_module_ids';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'milkit_widget_module_ids', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'milkit_widget_module_ids', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'milkit_widget_module_ids' ] ) ) {
			delete_option( 'milkit_widget_module_ids' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'milkit_widget_module_ids', 'widget' );
	}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'ids' ) ); ?>"><?php _e( 'ID(s):', 'milkit' ); ?></label>
			...
			<br>
			<small><?php _e( 'Type a list of posts IDs separated by commas (eg. 1,5,8).', 'milkit' ); ?></small>
		</p>
...
add_action( 'widgets_init', 'milkit_widget_module_ids' );

function milkit_widget_module_ids() {
	register_widget( 'Milkit_Widget_Module_IDs' );
}
