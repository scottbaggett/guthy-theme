...
class Milkit_Widget_Module_Recent extends WP_Widget {

	public function __construct() {
		$widget_ops = array(
			'classname' => 'milkit_widget_module_recent',
			'description' => __( 'Your site\'s most recent Posts.', 'milkit' )
		);
		parent::__construct( 'milkit_widget_module_recent', __( 'Milkit Post Modules - By Date', 'milkit' ), $widget_ops );
		$this->alt_option_name = 'milkit_widget_module_recent';
	...
	public function widget( $args, $instance ) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'milkit_widget_module_recent', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'milkit_widget_module_recent', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions[ 'milkit_widget_module_recent' ] ) ) {
			delete_option( 'milkit_widget_module_recent' );
		}
	...
	public function flush_widget_cache() {
		wp_cache_delete( 'milkit_widget_module_recent', 'widget' );
	}
...
add_action( 'widgets_init', 'milkit_widget_module_recent' );

function milkit_widget_module_recent() {
	register_widget( 'Milkit_Widget_Module_Recent' );
}
